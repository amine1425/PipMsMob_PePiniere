...
public class AffablerProducts extends BaseForm {
    ...
    public AffablerProducts() {
       ...
       List<Product> array = service.getListProd();

            for (int i = 0; i < array.size(); i++) {
                Product prod = array.get(i);
                ...
                b.addActionListener ( evt -> new  DetailProduit(res.getGlobalResources() ,prod).show () );

                Label lb = new Label(prod.getName());
                Label lb3 = new Label("" + prod.getId());
               Label qt = new Label("" + prod.getDescription());
                Label lb2 = new Label(String.valueOf(prod.getPrice()));
                Label qt2 = new Label(String.format("%s", prod.getDiscount_end_date()));
                eq1.add(lb);
                eq1.add(lb2);
                eq1.add(qt2);
                eq1.add(lb3);
